      module vars_module
        ...
        !parameter (NodeNum = 254102, NumEle=124050, NPT=1)
        !parameter (NodeNum = 217684, NumEle=162249, NPT=1)
        parameter (NodeNum = 8, NumEle= 1, NPT=1)
        !parameter (NodeNum = 8, NumEle=1, NPT=1)
        real*8,save :: allD(NodeNum), allH(NumEle), allDP(NumEle), allH_glo(NumEle),allD_glo(NodeNum)
        !dimension allD(NodeNum), allH(NumEle), allDP(NumEle), allH_glo(NumEle),allD_glo(NodeNum)
...
c
     ...
     *     third = 1.d0 / 3.d0, half = 0.5d0, op5 = 1.5d0)
      ...
      dimension dYield(nblkLocal), dHard(nblkLocal)
...
c       
      if ( nshr .gt. 1 ) then         ! 3D case
        ...
        do k = 1, nblock
          ...
          stateNew(k,1) = stateOld(k,1) + strainInc(k,1) - de_v_3
          stateNew(k,2) = stateOld(k,2) + strainInc(k,2) - de_v_3
          stateNew(k,3) = stateOld(k,3) + strainInc(k,3) - de_v_3
          stateNew(k,4) = stateOld(k,4) + strainInc(k,4)
          stateNew(k,5) = stateOld(k,5) + strainInc(k,5)
          stateNew(k,6) = stateOld(k,6) + strainInc(k,6)
...
c
     ...
     *     third = 1.d0 / 3.d0, half = 0.5d0, op5 = 1.5d0)
      ...
      parameter (nblkLocal = 136)
      dimension dYield(nblkLocal), dHard(nblkLocal), gdk(nblkLocal)
      dimension e_tensor(3,3), e_prin(3), e_voigt(6), e_dir(3,3), alphai(3)
      dimension s_tensor(3,3), s_prin(3,3)
      !real*8 phi, phiev, phiplus
*
...
c
     ...
     *         + twomu * strainInc(k,3) + alamda * trace
      ...
      do k=1 , nblock
          gdk(k) = (one-fieldOLd(k,1))**2
      enddo
      
      
      call xhard (nblock, stateOld(:,17), dYield, dHard, props(4), nvalue, gdk)
c       
      if ( nshr .gt. 1 ) then         ! 3D case
...
c
     ...
     *         + twomu * strainInc(k,3) + alamda * trace
      ...
      call xhard_jc (nblock, stateOld(:,17), dYield, dHard, props(4), nprops-3, 
     1 gdk, stateOld(:,20), tempNew, stateOld(:,21), dt)
c       
      if ( nshr .gt. 1 ) then         ! 3D case
...
c
          ...
          if (vmises > 0)then
            ...
            !dep23 = sqrt(op5)*deqps*s23/vmises
            !dep13 = sqrt(op5)*deqps*s13/vmises
            dep11 = op5*deqps*s11/vmises
            dep22 = op5*deqps*s22/vmises
            dep33 = op5*deqps*s33/vmises
            dep12 = op5*deqps*s12/vmises
            dep23 = op5*deqps*s23/vmises
            dep13 = op5*deqps*s13/vmises
          else
            dep11 = 0.d0
            dep22 = 0.d0
            dep33 = 0.d0
...
c Update the state variables
c
          stateNew(k,17) = stateOld(k,17) + deqps ! peeq
          !stateNew(k,20) = (stateOld(k,20) + deqps/dt)/2.0 ! strain rate
          stateNew(k,20) = deqps/dt ! strain rate
          stateNew(k,21) = yieldNew ! yield stress
...
!c Update the dissipated inelastic specific energy
          ...
          if (phase < d_thresh)then
              ...
              !if(nElem(k) == 147187 .or. nElem(k) == 147407)then
              !    ! write(*,*)nElem(k)
                  stateNew(k,14) = max( stateOld(k,14), max(stateNew(k,13) +  0.1*stateNew(k,19) - 0.1*w0, 0.0) ) ! H
              !else
              !    stateNew(k,14) = max( stateOld(k,14), stateNew(k,13) + max(1.0*stateNew(k,19) - 100.0, 0.0) ) ! H
              !endif
          else
              stateNew(k,14) = stateOld(k,14)
          endif
      ...
      subroutine xhard(nblock, peeqOld, syield, hard, table, nvalue, gdk)
      include 'vaba_param.inc'
...
c
 ...
 10     continue
      ...
      subroutine xhard_jc(nblock, peeqOld, syield, hard, table, nvalue, 
     1 gdk, e_rate, temp, yieldOld, dt)
      include 'vaba_param.inc'

c
      dimension peeqOld(nblock), syield(nblock), hard(nblock), gdk(nblock),
     *     table(nvalue), e_rate(nblock), temp(nblock), yieldOld(nblock)
c
      parameter(zero=0.d0)
      !A_jc = 0.1e3
      !B_jc = 1.072e3
...
c
      do k = 1, nblock
          ...
          if(peeqOld(k) < 0.0) peeqOld(k) = zero
          if(temp(k) < 0.0) temp(k) = zero
          
        syield(k) = (A_jc+B_jc*peeqOld(k)**en_jc)*(1.0+C_jc*log(max(e_rate(k),1.0)))*(1-(temp(k)/Tm)**em_jc)*gdk(k)
        hard(k) = zero
        if( peeqOld(k) > 0 ) hard(k) = en_jc*B_jc*peeqOld(k)**(en_jc-1)*(1.0+C_jc*log(max(e_rate(k),1.0)))*(1-(temp(k)/Tm)**em_jc)*gdk(k)
        
        !if(syield(k) < yieldOld(k))then
        !    syield(k) = yieldOld(k)
        !    hard(k) = zero
        !endif

        !if( e_rate(k)*dt > 0 )hard(k) = (syield(k) - yieldOld(k))/(e_rate(k)*dt)
        !if( peeqOld(k) > 0 ) hard(k) = en_jc*B_jc*peeqOld(k)**(en_jc-1)*(1.0+C_jc*log(max(e_rate(k),1.0)))*(1-(temp(k)/Tm)**em_jc)*gdk(k)
      end do
c    if more than one entry, search table
c
